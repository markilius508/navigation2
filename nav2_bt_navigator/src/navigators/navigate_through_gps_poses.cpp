  ...
  std::shared_ptr<nav2_util::OdomSmoother> odom_smoother)
{
  ...
  // Declare and get blackboard ID parameters
...
NavigateThroughGPSPosesNavigator::onLoop()
{
  ...
  try {
    ...
    // Calculate estimated time to goal if speed > 1cm/s and distance > 10cm
...
NavigateThroughGPSPosesNavigator::initializeGoalPoses(ActionT::Goal::ConstSharedPtr goal)
{
  if (goal->gps_poses.size() > 0) {
    RCLCPP_INFO(
      logger_, "NavThroughGPSPoses: Begin navigating from current location through %zu GPS poses",
      goal->gps_poses.size());
  }
